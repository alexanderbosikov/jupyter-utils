...
- **`{{ var }}` в `%%sql`** подставляется из пространства имён ноутбука: скаляры как есть (кавычки для строк пишутся руками, как в dbt), list/tuple/set — в SQL-кортеж для `in` с экранированием строк. См. `render_query` в `jupyter/magics.py`.
