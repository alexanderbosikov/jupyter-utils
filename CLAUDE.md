...
- **`{{ var }}` в `%%sql`** подставляется из пространства имён ноутбука: скаляры как есть (кавычки для строк пишутся руками, как в dbt), list/tuple/set — в SQL-кортеж для `in` с экранированием строк. См. `render_query` в `sql.py`.
- `run_query`/`run_file` клиентов принимают `connection=` (имя из конфига или объект опций; `None` = активное, резолвится через `options.resolve()` с проверкой типа) и `params=` — словарь для того же `{{ var }}`-рендеринга при вызове из python.
